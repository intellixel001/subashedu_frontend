...
interface Notification {
  _id: string;
  message: string;
  sentBy: StudentOption | string | null;
  sentTo: string;
  createdAt: string;
  readReceipt: boolean;
  sentByModel?: string;
...
export default function StudentNoti() {
  ...
  // Fetch notifications and student list
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [notiResponse, studentResponse] = await Promise.all([
          fetch(
        ...
        if (!studentResponse.ok) {
          ...
          console.error(
            `Student fetch failed: ${studentResponse.status} ${studentResponse.statusText}`
          );
          setNoStudents(true);
        }

        const notiContentType = notiResponse.headers.get("content-type");
        const studentContentType = studentResponse.headers.get("content-type");
        ...
        let studentResult = { success: false, data: [] };
        if (
          studentResponse.ok &&
          studentContentType?.includes("application/json")
        ) {
          studentResult = await studentResponse.json();
        } else {
          console.error(
            "Received non-JSON response from student API:",
            studentContentType || "No content-type"
          );
        }
        ...
        if (studentResult.success) {
          setStudentOptions(studentResult.data);
          setNoStudents(studentResult.data.length === 0);
        } else {
          setNoStudents(true);
        }
      } catch (error) {
  ...
  // Sort students: those with unread notifications go to the top
  ...
  const openSendModal = (student: StudentOption) => {
    setFormData({ message: "", sentTo: student._id });
  ...
  const handleReply = (notification: Notification) => {
    ...
    const student = studentOptions.find((s) => s._id === sentById);
    if (student) {
      setFormData({ message: "", sentTo: student._id });
      setSelectedStudent(student);
      setIsSendModalOpen(true);
  ...
  const openViewModal = async (student: StudentOption) => {
    setSelectedStudent(student);
    setIsViewModalOpen(true);

    // Update read receipt for notifications from this student
    const studentNotifications = getStudentNotifications(student._id);
    const unreadNotificationIds = studentNotifications
  ...
  const getStudentNotifications = (studentId: string) => {
    return notifications
      .filter((noti) => {
        const sentById =
          typeof noti.sentBy === "string" ? noti.sentBy : noti.sentBy?._id;
        return sentById === studentId || noti.sentTo === studentId;
      })
      .sort(
        (a, b) =>
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold flex items-center">
          <FaBell className="mr-2" /> Student Notifications
        </h1>
        <button
                  ...
                  const hasUnreadNotifications = notifications.some((noti) => {
                    const sentById =
                      typeof noti.sentBy === "string"
                        ? noti.sentBy
                        : noti.sentBy?._id;
                    return sentById === student._id && !noti.readReceipt;
                  });
                  return (
                    <tr key={student._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        {getStudentNotifications(student._id).length > 0 && (
                          <button
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    Send Notification to {selectedStudent?.fullName}
                  </Dialog.Title>
              ...
              >
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title
                    ...
                    className="text-lg font-medium leading-6 text-gray-900"
                  >
                    Messages with {selectedStudent?.fullName}
                  </Dialog.Title>
                  <div className="mt-4 max-h-96 overflow-y-auto">
                    {selectedStudent &&
                    getStudentNotifications(selectedStudent._id).length > 0 ? (
                      getStudentNotifications(selectedStudent._id).map(
                        (notification) => (
                          <div
                      ...
                      <p className="text-gray-500">
                        No messages with {selectedStudent?.fullName}.
                      </p>
                    )}
                  </div>
