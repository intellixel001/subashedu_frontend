    ...
    const fetchFreeClass = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/student/get-free-class/${classId}`,
          { credentials: "include" }
        );
        const result = await response.json();
        if (!response.ok) {
          throw new Error(result.message || "Failed to fetch free class");
        }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-red-500">
        ...
        <Link href={`/dashboard/free-classes/${name}`}>
          <button className="mt-4 bg-[#f7374f] text-white px-4 py-2 rounded-lg hover:bg-[#e12d42] transition-colors flex items-center gap-2">
            <FaArrowLeft /> Back to Free Classes
          </button>
        </Link>
      </div>
    );
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8 border border-gray-100">
        <div className="bg-[#f7374f] p-6 text-white">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <Link
              href={`/dashboard/free-classes/${name}`}
            ...
            <div className="md:col-span-2">
              ...
              <div className="space-y-4">
                <p>
                  ...
                  {selectedClass.classFor.charAt(0).toUpperCase() +
                    selectedClass.classFor.slice(1)}
                </p>
                <p>
            ...
            <div className="space-y-6">
              <div className="bg-[#f7374f]/5 p-4 rounded-lg border border-[#f7374f]/10">
                ...
                <p className="text-gray-600 text-sm mb-3">
                  This free class is available for all users. Use the custom
                  controls below the video to manage playback.
                </p>
              </div>
            </div>
          </div>
...
export default FreeClassVideoPlayer;
