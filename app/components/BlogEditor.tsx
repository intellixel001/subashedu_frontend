...
export function BlogEditor({ value, onChange }: BlogEditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "blog-editor prose max-w-none text-foreground p-4 min-h-[200px] focus:outline-none bg-card rounded-md dark:text-foreground dark:bg-card",
  ...
  useEffect(() => {
    if (editor && value !== editor.getHTML()) {
      editor.commands.setContent(value || "<p>Start writing your blog content...</p>");
    }
  }, [value, editor]);
  ...
  const handleFocus = () => {
    if (
      editor &&
      editor.getHTML() === "<p>Start writing your blog content...</p>"
    ) {
